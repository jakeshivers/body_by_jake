...
def facility_usage_silver():
    ...
    df_facility_usage_bronze = spark.read.parquet(f"{BRONZE_PATH}/{action}")
    df_members = spark.read.parquet(f"{SILVER_PATH}/members")

    df_silver = enforce_member_validity(df_facility_usage_bronze, df_members, timestamp_col="timestamp")

    logger.info(f"Read {df_facility_usage_bronze.count()} rows from BRONZE")

    df_silver = (
        df_silver
        .filter(col("member_id") > 0) 
        .filter(col("duration_mins") > 10) 
        .withColumn(f"{action}_date", col("timestamp"))
    )

    key_cols = ["usage_id", "member_id"]
