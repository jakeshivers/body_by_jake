...
def checkins_silver():
    ...
    df_checkins_bronze = spark.read.parquet(f"{BRONZE_PATH}/{action}")
    df_members = spark.read.parquet(f"{SILVER_PATH}/members")

    df_silver = enforce_member_validity(df_checkins_bronze, df_members, timestamp_col="timestamp")

    logger.info(f"Read {df_checkins_bronze.count()} rows from BRONZE")

    df_silver = (
        df_silver
        .filter(col("member_id").isNotNull())
        .withColumn("checkin_date", col("timestamp"))
    )

    key_cols = ["checkin_id", "member_id"]
