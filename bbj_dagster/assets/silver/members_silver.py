...
def members_silver():
    ...
    df_bronze = spark.read.parquet(f"{BRONZE_PATH}/{action}")
    logger.info(f"Read {df_bronze.count()} rows from BRONZE")
    ...
    df_silver = (
        ...
        .withColumn("join_date", col("timestamp").cast("date"))
        .drop("row_num")
    )

    key_cols = ["member_id", "join_date"]
